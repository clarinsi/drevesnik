...
class SetNode_Token(BaseNode):
    ...
    def __init__(self, token_restriction):
        ...
        elif self.token_restriction.startswith('X=') and not token_restriction.startswith('"'):
            self.token_restriction = self.token_restriction[2:]
            self.proplabel = '@CGXPOS'
        elif self.token_restriction.startswith('S=') and not token_restriction.startswith('"'):
            self.token_restriction = self.token_restriction[2:]
            self.proplabel = '@CGSIZE'
        else:
            self.proplabel = ''
